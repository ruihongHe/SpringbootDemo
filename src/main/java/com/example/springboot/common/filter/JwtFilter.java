...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (JedisUtil.exists(JwtConstant.PREFIX_SHIRO_REFRESH_TOKEN + username)) {
            ...
            if (JwtUtil.getClaim(token, JwtConstant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                JedisUtil.setObject(JwtConstant.PREFIX_SHIRO_REFRESH_TOKEN + username, currentTimeMillis, Integer.parseInt(refreshTokenExpireTime));
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = JSONObject.toJSONString(Response.setResult(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):" + msg, null));
            out.append(data);
        } catch (IOException e) {
            log.error("直接返回Response信息出现IOException异常:{}", e.getMessage());
            throw new CmsException(500, "直接返回Response信息出现IOException异常:" + e.getMessage());
        }
    }
